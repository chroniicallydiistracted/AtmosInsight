...
describe('GIBS URL builders', () => {
  it('builds tile URL with time', () => {
    ...
    expect(url).toBe('https://gibs.earthdata.nasa.gov/wmts/epsg3857/best/MODIS_Aqua_CorrectedReflectance_TrueColor/default/2025-08-27/GoogleMapsCompatible_Level9/3/1/2.jpeg');
  });
  ...
  it('builds tile URL without time', () => {
    ...
    expect(url).toBe('https://gibs.earthdata.nasa.gov/wmts/epsg3857/best/BlueMarble_ShadedRelief/default/GoogleMapsCompatible_Level8/3/1/2.jpeg');
  });
  ...
  it('builds domains URL', () => {
    ...
    expect(url).toBe('https://gibs.earthdata.nasa.gov/wmts/epsg3857/best/1.0.0/MODIS_Aqua_CorrectedReflectance_TrueColor/default/GoogleMapsCompatible_Level9/all/2025-08-01--2025-08-27.xml');
  });
});
...
describe('GIBS tile proxy', () => {
  ...
  it('serves AMSRE brightness temp tile', async () => {
    const res = await request(app).get('/api/gibs/tile/3857/AMSRE_Brightness_Temp_89H_Day/2011-10-04/GoogleMapsCompatible_Level6/3/1/2.png');
    expect(res.status).toBe(200);
...
describe('GIBS domains proxy', () => {
  it('passes through domains request', async () => {
    const res = await request(app).get('/api/gibs/domains/3857/MODIS_Aqua_CorrectedReflectance_TrueColor/GoogleMapsCompatible_Level9/2025-08-01--2025-08-27.xml');
    expect(res.status).toBe(200);
