...
app.get('/', async (req, res) => {
  ...
  try {
    ...
    res.status(HTTP_STATUS.BAD_GATEWAY).json(
      createErrorResponse(HTTP_STATUS.BAD_GATEWAY, 'Proxy error', {
...
app.use('/api/nws/alerts', shortLived60, async (req, res) => {
  ...
  try {
    ...
    res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(
      createErrorResponse(HTTP_STATUS.INTERNAL_SERVER_ERROR, 'Proxy error', {
        originalError: err instanceof Error ? err.message : String(err),
      })
    );
  }
    ...
    try {
      ...
      if (!targetUrl) {
        res
          ...
          .json(createErrorResponse(HTTP_STATUS.NOT_FOUND, 'Frame not found'));
        return;
      }
...
function redirectGibs(req: express.Request, res: express.Response) {
  ...
  if (!url.startsWith('https://gibs.earthdata.nasa.gov/')) {
    res
      .status(HTTP_STATUS.BAD_REQUEST)
      .json(
        createErrorResponse(HTTP_STATUS.BAD_REQUEST, 'Invalid redirect URL')
      );
    return;
  }
...
function ensureGibsEnabled(
  ...
  if (
    ...
    res
      .status(HTTP_STATUS.SERVICE_UNAVAILABLE)
      .json(
        createErrorResponse(HTTP_STATUS.SERVICE_UNAVAILABLE, 'GIBS disabled')
      );
    return;
  }
  next();
...
function ensureAirNowEnabled(
  ...
  if (
    ...
    res
      .status(HTTP_STATUS.SERVICE_UNAVAILABLE)
      .json(
        createErrorResponse(HTTP_STATUS.SERVICE_UNAVAILABLE, 'AirNow disabled')
      );
    return;
  }
  next();
...
function ensureOpenAqEnabled(
  ...
  if (
    ...
    res
      .status(HTTP_STATUS.SERVICE_UNAVAILABLE)
      .json(
        createErrorResponse(HTTP_STATUS.SERVICE_UNAVAILABLE, 'OpenAQ disabled')
      );
    return;
  }
  next();
...
// -----------------
app.get(
  '/api/tracestrack/:style/:z/:x/:y.webp',
  shortLived60,
  async (req, res) => {
    try {
      ...
      const targetUrl = `https://tile.tracestrack.com/${style}/${z}/${x}/${y}.webp?key=${apiKey}&style=${styleParam}`;
  ...
  app.post('/api/glm-toe/ingest', express.json(), (req, res) => {
    try {
      ...
      res.status(HTTP_STATUS.OK).json(
        createSuccessResponse(HTTP_STATUS.OK, {
          ok: true,
          count: batch.length,
        })
      );
