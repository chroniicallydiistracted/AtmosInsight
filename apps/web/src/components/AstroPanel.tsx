  ...
  return (
    <div className="panel absolute top-4 left-4 p-3 text-sm">
      ...
      <div className="space-y-1 text-xs">
        ...
        <div>
          Moon: az {deg(astro.moon.azimuth_rad).toFixed(1)}°, el {deg(astro.moon.elevation_rad).toFixed(1)}°, 
          dist {(astro.moon.distance_m/1000).toFixed(0)} km, phase {Math.round(astro.moon.phase_fraction * 100)}%
        </div>
      </div>
    </div>
  );
