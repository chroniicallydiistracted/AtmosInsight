...
# AtmosInsight Monorepo Status Check Script
# This script shows the current status of all services in a pnpm monorepo
...
echo -e "${BLUE}📊 AtmosInsight Monorepo Status Check${NC}"
echo "======================================="
echo ""
...
if [ ! -f "package.json" ] || [ ! -d "apps" ] || [ ! -d "proxy-server" ] || [ ! -d "tiling-services" ]; then
    ...
    echo -e "${RED}💡 Please run: cd /path/to/AtmosInsight && ./status-atmosinsight-monorepo.sh${NC}"
    exit 1
fi
...
if check_service $SHARED_PORT "$PROXY_HEALTH"; then
    details=$(get_port_details $SHARED_PORT)
...
if check_service $SHARED_PORT "$CATALOG_HEALTH"; then
    details=$(get_port_details $SHARED_PORT)
    IFS='|' read -r pid cmd user time <<< "$details"
...
if [ "$PROXY_RUNNING" = true ] && [ "$CATALOG_RUNNING" = true ] && [ "$WEB_RUNNING" = true ]; then
    ...
    echo -e "${YELLOW}⚠️  Only Web App is running${NC}"
    echo -e "${BLUE}💡 Start proxy-server: pnpm --filter proxy-server run dev${NC}"
    echo -e "${BLUE}💡 Start catalog-api: pnpm --filter catalog-api run start${NC}"
else
    echo -e "${RED}❌ No services are running${NC}"
    echo -e "${BLUE}💡 Start all services: ./start-atmosinsight-monorepo.sh${NC}"
fi
...
echo "=================="
echo -e "${GREEN}./start-atmosinsight-monorepo.sh${NC}  - Start all services"
echo -e "${RED}./stop-atmosinsight-monorepo.sh${NC}   - Stop all services"
echo -e "${BLUE}./status-atmosinsight-monorepo.sh${NC} - Show this status (current script)"
echo ""
