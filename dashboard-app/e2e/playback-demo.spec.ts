...
test('Radar playback demo (records video)', async ({ page }) => {
  ...
  await page.waitForFunction(
    () =>
      ...
      !!(window as unknown as { __map?: { getCanvas?: () => unknown } })
        .__map &&
      typeof (window as unknown as { __map?: { getCanvas?: () => unknown } })
        .__map!.getCanvas === 'function' &&
      !!(
        window as unknown as { __map?: { getCanvas?: () => unknown } }
      ).__map!.getCanvas(),
    { timeout: 30000 }
  ...
  await page.waitForEvent(
    'response',
    r => /\/api\/rainviewer\/.+\.png$/.test(r.url()) && r.ok(),
    { timeout: 15000 }
  );
  ...
  void (await page.evaluate(
    () => (window as unknown as { __rrq?: number }).__rrq ?? 0
  ));
